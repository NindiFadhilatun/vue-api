...
// envelope is the type used to give label to jsonResponse data
...
// Logout is the handler used to attempt to log out a user from the api
...
// DeleteUser deletes a user from the users' table by the id given in the supplied JSON file
...
func (app *application) AuthorsAll(w http.ResponseWriter, _ *http.Request) {
	...
	for _, authorSlice := range authors {
		author := selectData{
			Value: authorSlice.ID,
			Text:  authorSlice.AuthorName,
		}
